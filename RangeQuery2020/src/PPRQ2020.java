...
public class PPRQ2020{
	
	

	//Range Query Parameters
	static int N=500;//number of IoT devices.
	static int m=20;//m=log2(n) or 2^m=n
	static int n=(int) Math.pow(2, m);//range query upper bound. //Leaf nodes 2^{0,1,2,3,4,5,6,7,...}=1,2,4,8,16,32,64,128,...
    static int range[] = {1,n/2};//{1,n-2}; //{0,7,16,19,24,24,28,29};
        //[Ls.Us] = [L1.U1] U [L2.U2] U ... U [Ls.Us]
	static int []w= new int[N];//wi is a prepared data from IoT device Di and it is btw. 0 and n-1.
    ...
    public static void main(String args[]) {
    	...
    	//nge = new int []{0,7,11,11,28,29};
	        ...
	        */
			...
			//So we have (E*E)*(E*E)*(E*E)*(E*E) ==> we need (m)+(m-1) multiplication, so far.
			//Since, we need (2m-1+1)*k2 << k0, therefore we set k0=k0+k0*15% 
	    	...
	    	BigInteger[] E0 = new BigInteger[N];
	    	BigInteger[] E1 = new BigInteger[N];
	    	for(int i=0; i<N; i++) {
	    	...
	    	for (int i=0; i<N; i++) {//Generate random value for each IoT devices Di, 0<=i<=N-1.
	    		...
	    		for(int j=0; j<m; j++) {
	    			if(wBinary[i].charAt(j) == '0') {
	    				tmpEBInt[j] = E0[i].multiply(BigInteger.valueOf(rand.nextInt(999999-1)+1));
	    				//tmpEBInt[j] = shs.Mul(E0[i], BigInteger.valueOf(rand.nextInt(999999-1)+1), pp);
	    			}else if(wBinary[i].charAt(j) == '1') {	    				
	    				tmpEBInt[j] = E1[i].add(E0[i].multiply(BigInteger.valueOf(rand.nextInt(999999-1)+1)));
	    				//tmpEBInt[j] = shs.Mul(E1[i], BigInteger.valueOf(rand.nextInt(999999-1)+1), pp);
	    			}	    			
	    		}
	    	...
	    	elapsedTime = endTime-startTime;
	    	System.out.println("Response at each IoT (XOR) (ns):"+elapsedTime+"/"+N+": "+elapsedTime/N);
	    	...
	    	endTime=System.nanoTime();
	    	elapsedTime = endTime-startTime;
	    	System.out.println("Response at each IoT (Mul) (ns):"+elapsedTime+"/"+N+": "+elapsedTime/N);
	    	...
	    	endTime=System.nanoTime();
	    	elapsedTime = endTime-startTime;
	    	System.out.println("Fog Node: aggregate each IoT response (ns):"+elapsedTime);
	    	...
	    	endTime = System.nanoTime();
	    	elapsedTime = endTime-startTime;
	    	System.out.println("Fog Node: aggregate all the final responses (ns):"+elapsedTime);
	    	...
	    	endTime = System.nanoTime();
	    	elapsedTime = endTime-startTime;
	    	System.out.println("Response recover at user side (ns):"+elapsedTime);
