...
public final class SymHomSch {
 ...
 public BigInteger Dec(BigInteger cipher, SHSParamters Param) {
    	...
    	//HASSAN_COMMENT_FOR_PPRQ2020 System.out.println("cipher.BitLength: " + cipher.bitLength());
    	BigInteger x = cipher.mod(p);
    	BigInteger y;
    	//HASSAN_COMMENT_FOR_PPRQ2020 System.out.println("x.BitLength: " + x.bitLength());
    	...
    	{// if it had been set to k1+15 it can recover m values grater than message space.
    		y = x.mod(L);
    		//HASSAN_COMMENT_FOR_PPRQ2020 System.out.println("y.BitLength: " + y.bitLength());
    		//HASSAN_COMMENT_FOR_PPRQ2020 System.out.println("Case -1");
    	...
    	}else if ((x.bitLength() > k2+5) &&  (x.bitLength() < k0-50))// x = c mod p; |L| < |x| << |p|    	
    	{
    		...
    		{
    			//HASSAN_COMMENT_FOR_PPRQ2020 System.out.println("Case 2.1");
    			return y.subtract(L);
    		}
    		else
    		{
    			//HASSAN_COMMENT_FOR_PPRQ2020 System.out.println("Case unknown01 - due to bad boundary setting.");
    			return BigInteger.valueOf((long)-1);
    		}
    	}else if((x.bitLength()<k0+5 && x.bitLength()>k0-5))// x = c mod p; x close to p
    	{
    		...
    		{
    			//HASSAN_COMMENT_FOR_PPRQ2020 System.out.println("Case 3");
    			return y.subtract(p.mod(L));
    		}
    		else if(y.compareTo(p.mod(L))==-1)// y < p mod L
    		{
    			//HASSAN_COMMENT_FOR_PPRQ2020 System.out.println("Case 4 or 2.2");
    			return y.subtract(p.mod(L));
    		}
    		else
    		{
    			//HASSAN_COMMENT_FOR_PPRQ2020 System.out.println("Case unknown02");
    			return BigInteger.valueOf((long)-1);
    		}    	
    	}
    	else
    	{
    		//HASSAN_COMMENT_FOR_PPRQ2020 System.out.println("Case unknown03");
			return BigInteger.valueOf((long)-1);
    	}
    }
    /*
    ...
    */
    ...
    public static BigInteger Add(BigInteger cipher1, BigInteger cipher2, SHSParamters Param)
    {
    	...
    	cipher1 = cipher1.mod(Param.getParams().get(0));
    	cipher2 = cipher2.mod(Param.getParams().get(0));
